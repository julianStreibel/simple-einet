...
class EinsumLayerProdDropout(AbstractLayer):
    def __init__(
        self,
        num_features: int,
        num_sums_in: int,
        ...
        self._sum_binomial_dist = torch.distributions.binomial.Binomial(
            self.num_sums_in**2 - 1, self.sum_dropout)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.sum_dropout > 0. and self.training:
            n_dropout = self._sum_binomial_dist.sample().int().item()
            ...
            # broadcast in sum_out dim
            ninf = ninf.expand(-1, self.num_sums_out, -1, -1)
