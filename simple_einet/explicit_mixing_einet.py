...
from simple_einet.factorized_leaf_layer import FactorizedLeafPlusCategorical
...
class ExplicitMixingEinet(Einet):
    ...
    def __init__(self, config: EinetConfig):
        ...
        # setting number of classes to one because we only model the joint
        config.num_classes = 1
        super().__init__(config)

    def forward(self, x: torch.Tensor, y: torch.Tensor = None, marginalization_mask: torch.Tensor = None, sinkhorn_tau=None) -> torch.Tensor:
        """
        ...
        x = self.leaf(x, y, marginalization_mask, tau=sinkhorn_tau)
