...
class ClassConditionalFactorizedLeaf(AbstractLayer):
    """ 
    This layer is a FactorizedLeaf conditioned on the class variable.
    """
    ...
    def __init__(
        ...
        num_features_out: int,
        num_repetitions,
        base_leaves: List[AbstractLeaf],
        class_idx: int):
        """
        Args:
            num_features (int): Number of input features/RVs.
            num_features_out (int): Number of output features/RVs.
            num_repetitions (int): Number of repetitions.
            base_leaves (list of Leaves): list of base leaf distribution object.
        """

        self.factorized_leaves = list()
        for base_leaf in base_leafs:
            factorized_leaf = FactorizedLeaf(
                num_features=base_leaf.out_features,
                num_features_out=num_features_out,
                num_repetitions=num_repetitions,
                base_leaf=base_leaf,
            )
            self.factorized_leves.append(factorized_leaf)


    def forward(self, x: torch.Tensor, marginalized_scopes: List[int]):
        y = x[self.class_idx] # TODO: y could be nan and needs to be summed out
        return self.factorized_leaves[y].forward(x, marginalized_scopres)

    def sample(self, num_samples: int = None, context: SamplingContext = None) -> torch.Tensor:
        y = x[self.class_idx] # TODO: y could be nan and needs to be summed out
        return self.factorized_leaves[y].sample(num_samples, context)