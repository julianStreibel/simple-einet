...
class CCRatNormal(AbstractLeaf):
    ...
    def __init__(
        ...
        # Create gaussian means and stds
        self.means = nn.Parameter(torch.randn(1, num_channels, num_features, num_leaves, num_repetitions, num_classes))

        if min_sigma is not None and max_sigma is not None:
            # Init from normal
            self.stds = nn.Parameter(torch.randn(1, num_channels, num_features, num_leaves, num_repetitions, num_classes))
        else:
            # Init uniform between 0 and 1
            self.stds = nn.Parameter(torch.rand(1, num_channels, num_features, num_leaves, num_repetitions, num_classes))
...
class CustomCCNormal:
    ...
    def sample(self, sample_shape: Tuple[int]):
        ...
        # samples = self.mu.unsqueeze(0) + self.sigma.unsqueeze(0) * eps
        # return samples
        raise NotImplementedError("Sampling in CustomCCNormal not implemented jet.")

    def log_prob(self, x):
        
        # remove and save class variable from features
        class_feature = x[:,:, self.class_idx,:,:].flatten().to(torch.int64)
        one_hot_class_feature = F.one_hot(class_feature, self.num_classes).float()
        num_features = x.shape[2]
        x_without_class_feature = x[:, :, np.r_[:self.class_idx,self.class_idx+1:num_features],:,:]
        

        # adding class dimension:
        # - add dimension in most inner dimension
        # - repeat data in in last dimension num_classes times
        x_without_class_feature = (
                x_without_class_feature
                    .unsqueeze(-1)
                    .repeat([1 for _ in x.shape] + [self.num_classes])
            )

        log_likelihood_for_all_classes = dist.Normal(self.mu, self.sigma).log_prob(x_without_class_feature)

        # remove class dimension and set to log_likelihood of wanted class
        # using einsum over the class dim with one hot encoded class labels and the log likelihoods
        # to remove log likelihoods of unwanted classes
        # indices:
        # - s: sample in batch
        # - C: channels
        # - f: features
        ...
        conditional_log_likelihood = torch.einsum("sCflrc, sc -> sCflr", log_likelihood_for_all_classes, one_hot_class_feature)
