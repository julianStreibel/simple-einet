...
class CustomCategorical(nn.Module):
    ...
    def __init__(
        ...
        self.p = nn.Parameter(torch.randn(1, num_leaves, num_repetitions, num_classes))

    def forward(self, y):

        p = nn.functional.softmax(self.p, dim=2)
        dist = Categorical(p)
    ...
    def sample(self, context: SamplingContext = None):
        ...
        if context.is_mpe or context.mpe_at_leaves:
            return p[context.indices_repetition].argmax(dim=2)
