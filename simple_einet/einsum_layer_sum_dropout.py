...
class EinsumLayerSumDropout(AbstractLayer):
    def __init__(
        self,
        num_features: int,
        num_sums_in: int,
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.sum_dropout > 0. and self.training:
            # get dropout indices
            dropout_indices = self._sum_bernoulli_dist.sample(
                weights.shape).bool().cuda()

            # check if at least one component is active
            indices_all_dropout = dropout_indices.all(
                dim=-1, keepdim=True).expand(weights.shape).cuda()

            # force one component to be active
            replacement = ~torch.nn.functional.one_hot(torch.rand_like(
                weights).argmax(dim=-1), self.num_sums_in * self.num_sums_in).bool().cuda()

            # set one component to 1 if all zero in one sum
            dropout_indices = torch.where(
                indices_all_dropout, replacement, dropout_indices)

            # create dropout values
            ninf = torch.zeros_like(weights)
            ninf[dropout_indices] = np.NINF
            # set some component probs to zero
    ...
    def extra_repr(self):
        return "num_features={}, num_sums_in={}, num_sums_out={}, num_repetitions={}, out_shape={}, sum_dropout={}, " "weights_shape={}".format(
            self.num_features,
            self.num_sums_in,
            self.num_sums_out,
            self.num_repetitions,
